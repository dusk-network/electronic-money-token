...
use dusk_core::abi::{ContractError, Metadata, StandardBufSerializer};
...
impl MainnetSession {
    ...
    pub fn icc_transaction(
        ...
        fn_name: &str,
        fn_arg: Vec<u8>,
    ) -> CallReceipt<std::result::Result<Vec<u8>, ContractError>>
//where
        //A: for<'b> Serialize<StandardBufSerializer<'b>>,
        //A::Archived: for<'b> CheckBytes<DefaultValidator<'b>>,
        //R: Archive,
        //R::Archived: Deserialize<R, Infallible> + for<'b> CheckBytes<DefaultValidator<'b>>,
    {
        ...
        // TODO: this function should return the generic R type of the .call function used in it.
        let receipt = execute(&mut self.session, &transaction, &self.config)
            .unwrap_or_else(|e| panic!("Executing the transaction should succeed: {:?}", e));
        /*fn deserialize<T>(receipt: CallReceipt<Vec<u8>>) -> CallReceipt<T>
        where
            T: Archive,
            T::Archived: Deserialize<T, Infallible> + for<'b> CheckBytes<DefaultValidator<'b>>,
        {
            let ta = check_archived_root::<T>(&receipt.data[..]).unwrap();
        ...
        let receipt = deserialize::<R>(CallReceipt {
            gas_spent: receipt.gas_spent,
            gas_limit: receipt.gas_limit,
            events: receipt.events,
            call_tree: receipt.call_tree,
            data: receipt.data.unwrap(),
        });*/
...
impl MainnetSession {
    ...
    pub fn instantiate(pks_to_fund: Vec<(&AccountPublicKey, u64)>) -> Self {
        ...
        // fund public keys with moonlight dusk
        ...
        for (pk, _) in pks_to_fund {
            // the moonlight account is instantiated with the expected value
            let sender_account =
                account(&mut session, &pk).expect("Getting the sender account should succeed");
